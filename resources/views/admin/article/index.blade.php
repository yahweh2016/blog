@extends('layout/admin')
        @section('contents')
    <!--面包屑导航 开始-->
    <div class="crumb_warp">
        <!--<i class="fa fa-bell"></i> 欢迎使用登陆网站后台，建站的首选工具。-->
        <i class="fa fa-home"></i> <a href="#">首页</a> &raquo; <a href="#">商品管理</a> &raquo; 添加商品
    </div>
    <!--面包屑导航 结束-->

	<!--结果页快捷搜索框 开始-->
	<div class="search_wrap">
        <form action="" method="post">
            <table class="search_tab">
                <tr>
                    <th width="120">选择分类:</th>
                    <td>
                        <select onchange="javascript:location.href=this.value;">
                            <option value="">全部</option>
                            @foreach($cate as $cate)
                                <option value="http://www.sina.com">{{$cate->cate_name}}</option>
                            @endforeach
                        </select>
                    </td>
                    <th width="70">关键字:</th>
                    <td><input type="text" name="keywords" placeholder="关键字"></td>
                    <td><input type="submit" name="sub" value="查询"></td>
                </tr>
            </table>
        </form>
    </div>
    <!--结果页快捷搜索框 结束-->

    <!--搜索结果页面 列表 开始-->
    <form action="#" method="post">
        <div class="result_wrap">
            <!--快捷导航 开始-->
            <div class="result_content">
                <div class="short_wrap">
                    <a href="#"><i class="fa fa-plus"></i>新增文章</a>
                    <a href="#"><i class="fa fa-recycle"></i>批量删除</a>
                    <a href="#"><i class="fa fa-refresh"></i>更新排序</a>
                </div>
            </div>
            <!--快捷导航 结束-->
        </div>

        <div class="result_wrap">
            <div class="result_content">
                <table class="list_tab">
                    <tr>
                        <th class="tc" width="5%"><input type="checkbox" name=""></th>
                        <th class="tc">排序</th>
                        <th class="tc">ID</th>
                        <th>标题</th>
                        <th>点击</th>
                        <th>发布人</th>
                        <th>更新时间</th>
                        <th>评论</th>
                        <th>操作</th>
                    </tr>
                    @foreach($art as $article)
                    <tr>
                        <td class="tc"><input type="checkbox" name="id[]" value="59"></td>
                        <td class="tc">
                            <input type="text" name="art_order" value="{{$article['art_order']}}">
                        </td>
                        <td class="tc">{{$article['art_id']}}</td>
                        <td>
                            <a href="#">{{$article['art_title']}}</a>
                        </td>
                        <td>{{$article['art_view']}}</td>
                        <td>{{$article['art_editor']}}</td>
                        <td>{{date('Y-m-d H:i:s',$article['art_time'])}}</td>
                        <td></td>
                        <td>
                            <a href="{{url('admin/article/'.$article['art_id'].'/edit')}}">修改</a>
                            <a href="javascript:" onclick="dele('{{$article->art_id}}')">删除</a>
                        </td>


                        <script>
                            function dele(art_id){
                                var url = '{{url('admin/article')}}/'+art_id;
                                layer.confirm(
                                        '你确定要删除该分类吗？',
                                        {
                                            btn: ['确定','取消'] //按钮
                                        }, function(){
                                            $.ajax({
                                                url:url,
                                                type:'post',
                                                dataType:'json',
                                                data:{
                                                    'art_id':art_id,
                                                    '_method':'delete',
                                                    '_token':'{{csrf_token()}}'
                                                },
                                                success:function(response){
                                                    if(response.error == '0'){
                                                        location.href = '{{url('admin/article')}}';
                                                        layer.msg(response.msg,{icon:6});

                                                    }else{
                                                        layer.msg(response.msg,{icon:5});
                                                    }
                                                }
                                            })
                                        },
                                        function(){
                                            layer.msg('感谢请的手下留情', {icon:5});
                                        }
                                )}
                        </script>
                    </tr>
                        @endforeach
                </table>
                <div class="page_list">
                  {!! $art->links() !!}
                    <style>

                        .page_list ul li span {
                            padding: 6px 12px;
                            text-decoration: none;
                        }
                    </style>
                </div>
            </div>
        </div>
    </form>
    <!--搜索结果页面 列表 结束-->
@endsection